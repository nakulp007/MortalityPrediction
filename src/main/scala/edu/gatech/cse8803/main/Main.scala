...
object Main {
  def main(args: Array[String]) {
    ...
    val (notes, firstNoteDates) = processNotesAndCalculateStartDates(
      patients1, icuStays1, rawNotes)
    println(s"firstNoteDates count: ${firstNoteDates.count}")

    // Filter out patients that have start dates that aren't from notes (meaning
    // they had no notes)
    val (patients, icuStays) = filterOutPatientIcuWithoutFirstNote(
      patients1, icuStays1, firstNoteDates)
    //patients.foreach(println)
    println(s"Patients with first note count: ${patients.count}")
  ...
  def runRetrospectiveTopicModel(sc: SparkContext,
    ...
    val numIterationsParams: List[Int] = List(200) // 100 is default
    val regParams: List[Double] = List(0.1) // 0.0 is default
